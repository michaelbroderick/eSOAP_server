...
const addCheck = (label) => {
    ...
    let val = checkValue(`[name=${label.htmlFor}]`)
    console.log(label.htmlFor, val)
    if (val || label.parentElement.classList.contains('input_visibility')) {
        ...
        for (let input of inputs) {
            ...
            const checkCheck = () => {
                ...
                for (let h2 of inputHeadings) {
                    let val = checkValue(`[name=${h2.querySelector('label').htmlFor}]`)
                    if (val || h2.classList.contains('input_visibility')) {
                        currCount += 1;
                    }
                }
